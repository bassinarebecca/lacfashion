...
<div class="droite  card col ">
    <div class="card-header text-center font-weight-bold"> <i class="text-info fas fa-edit"></i> Modifier {{$produit->nom}}</div>
    <div class="card-body">
        <form method="post" action="{{route('produit.edit', $produit->id)}}" id="tr_form" enctype="multipart/form-data" >
            @csrf
            @method('PATCH')
            <div class="form-group row">
                <label for="code" class="col-12 col-lg-2 text-right control-label col-form-label">Nom:</label>
                <div class="col ">
                    <input id="code" type="text" class="form-control @error('nom') is-invalid @enderror" name="nom" value="{{ old('nom') ?? $produit->nom }}"  placeholder="ajouter le nom ">
                    @error('nom')
